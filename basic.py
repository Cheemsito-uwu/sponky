...
from data.strings_with_arrows import *
from data import set
from data.pyinclude import colors as color
from data.pyinclude import get_js as getjs
...
class Parser:
  ...
  def atom(self):
    ...
    elif tok.matches(TT_KEYWORD, 'build'):
      build_def = res.register(self.build_def())
      if res.error: return res
      return res.success(build_def)
    
    elif tok.matches(TT_KEYWORD, 'switch'):
      switch_def = res.register(self.switch_def())
      if res.error: return res
      return res.success(switch_def)
  ...
  def switch_def(self):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'switch'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'switch'"
      ))
  ...
  def build_def(self):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'build'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'build'"
      ))
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_int(self, exec_ctx):
    thing = str(exec_ctx.symbol_table.get("thing"))
    thing = int(thing)
  ...
  def execute_string(self, exec_ctx):
    obj = str(exec_ctx.symbol_table.get("obj"))
  ...
  def execute_sp4math(self, exec_ctx):
    mode = str(exec_ctx.symbol_table.get('mode'))
    int_1 = str(exec_ctx.symbol_table.get('int1'))
    int_2 = str(exec_ctx.symbol_table.get('int1'))

    Math = getjs.file("mathing.js")
    calculator = getjs.js2py.run_file("data\\js-scripts\\calculator.js")

    if mode == "sqrt":
      int1 = int(int_1)
      int2 = int(int_2)

      rs = Math.make_sqrt(int1)
      return RTResult().success(
        Number(rs)
      )
    elif mode == "hypotenuse":
      int1 = int(int_1)
      int2 = int(int_2)

      rs = math.sqrt((int1 * int1) + (int2 * int2))
      return RTResult().success(
        Number(rs)
      )
    elif mode == "calculate":
      ...
      return RTResult().failure(
        RTError(
          self.pos_start, self.pos_end,
          f"The 'math mode' you specified does not exists"
        )
      )
  execute_sp4math.arg_names = ["mode", "int1", "int2", "int3"]

  def execute_co1wind(self, exec_ctx):
    id = str(exec_ctx.symbol_table.get('id'))
    
    with open('data/temp/ids/json/ids.json', 'r') as f:
      ids = json.load(f)

    if any(identification in id for identification in ids):
      identi = id
      if ids['ids'][id] == True:
        return RTResult().failure(
          Number(1)
  ...
  execute_co1wind.arg_names = ["id"]

  def id_set(value:bool=True):
    if value == True:
      with open('data/temp/ids/json/ids.json', 'r') as f:
        ids = json.load(f)
      ...
      with open('data/temp/ids/json/ids.json', 'w') as f:
        json.dump(ids, f)
    else:
      pass
  ...
  def execute_ad1wind(self, exec_ctx):
    ...
    if type == "button":
      global button
      DoubleVar
      button = Button(window, text=text, command=lambda: self.id_set)
      button.pack()
    elif type == "label":
      global label
...
BuiltInFunction.make    = BuiltInFunction("make")
BuiltInFunction.rand    = BuiltInFunction("rand")
BuiltInFunction.pass_   = BuiltInFunction("pass")
BuiltInFunction.use     = BuiltInFunction("use")
BuiltInFunction.def_    = BuiltInFunction("def")
BuiltInFunction.get    = BuiltInFunction("get")
BuiltInFunction.strequal = BuiltInFunction("strequal")
BuiltInFunction.case = BuiltInFunction("case")

BuiltInFunction.int = BuiltInFunction("int")
BuiltInFunction.string = BuiltInFunction("string")
BuiltInFunction.wf1daffl = BuiltInFunction("wf1daffl")
...
BuiltInFunction.co1wind = BuiltInFunction("co1wind")
BuiltInFunction.po1cons = BuiltInFunction("po1cons")
BuiltInFunction.ge2keys = BuiltInFunction("ge2keys")
BuiltInFunction.er3inte = BuiltInFunction("er3inte")
BuiltInFunction.sp4math = BuiltInFunction("sp4math")
...
global_symbol_table.set("def", BuiltInFunction.def_)
global_symbol_table.set("get", BuiltInFunction.get)
global_symbol_table.set("strequal", BuiltInFunction.strequal)
global_symbol_table.set("str_equal", BuiltInFunction.strequal)
global_symbol_table.set("case", BuiltInFunction.case)

global_symbol_table.set("wf1daffl", BuiltInFunction.wf1daffl)
...
global_symbol_table.set("co1wind", BuiltInFunction.co1wind)
global_symbol_table.set("po1cons", BuiltInFunction.po1cons)
global_symbol_table.set("ge2keys", BuiltInFunction.ge2keys)
global_symbol_table.set("er3inte", BuiltInFunction.er3inte)
global_symbol_table.set("sp4math", BuiltInFunction.sp4math)

# Libraries
def save_location():
  if set.check.user_json() == True:
    pass
  else:
    set.location()
  ...
  str_sys = runs['location']['libs']['sys']
  str_math = runs['location']['libs']['math']
  str_console = runs['location']['libs']['console']
  str_files = runs['location']['libs']['files']
  str_init = runs['location']['libs']['init']

  String_sys = String(str_sys)
  String_math = String(str_math)
  String_console = String(str_console)
  String_files = String(str_files)
  String_init = String(str_init)

  global_symbol_table.set("sys", String_sys)
  global_symbol_table.set("system", String_sys)
  global_symbol_table.set("console", String_console)
  global_symbol_table.set("cons", String_console)
  global_symbol_table.set("interpreter", String_console)
  global_symbol_table.set("math", String_math)
  global_symbol_table.set("files", String_files)
  global_symbol_table.set("file", String_files)
  global_symbol_table.set("init", String_init)
